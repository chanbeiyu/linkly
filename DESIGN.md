...
Linkly 是一个面向收藏、整理、检索和后台维护的书签管理工具。界面应当像一张安静、可靠、可长期停留的工作台：内容密度高但不压迫，视觉有品质但不抢戏，操作路径清楚，状态反馈及时。
...
Linkly 的默认界面气质是 **清透的实用主义**。它可以有轻微玻璃质感、柔和边框和细腻过渡，但不做营销式大卡片堆叠，不做过度插画，不做紫蓝渐变铺满页面，不让装饰元素稀释信息。
...
Linkly 的核心体验以桌面端为主，移动端保证核心内容浏览、搜索、编辑和提交可用。
...
Linkly 现有页面存在较柔和的圆角语言，但新增工具型组件应优先克制。
...
- 可以更有品牌感，但第一屏必须让用户看到 Linkly 是书签管理工具。
